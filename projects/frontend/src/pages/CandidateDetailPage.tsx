...
export function CandidateDetailPage() {
  ...
  async function saveResumeProfile() {
    ...
    const result = await saveCandidate(candidate.id, {
      ...values,
      education: (values.education || []).map(({ school, degree, graduate_at }) => ({
        school, degree, graduate_at,
      })),
      work_experience: (values.work_experience || []).map(({ company, start, end }) => ({
        company, start, end,
      })),
    });
              ...
              <div>
                <Space align="center" size={10} wrap>
                  ...
                  <Dropdown
                    trigger={['click']}
                    menu={{
                      items: [...detail.applications].reverse().map((application, index) => ({
                        key: String(application.id),
                        label: (
                          <div style={{ minWidth: 260 }}>
                            <Typography.Text strong>第{index + 1}次投递</Typography.Text>
                            <div style={{ color: '#8c8c8c', marginTop: 2 }}>
                              {application.job_name || '未关联职位'} · {application.created_at || application.stage_entered_at || '-'}
                            </div>
                          </div>
                        ),
                      })),
                      selectable: true,
                      selectedKeys: selectedApplication ? [String(selectedApplication.id)] : [],
                      onClick: ({ key }) => setSelectedAppId(Number(key)),
                    }}
                  >
        ...
        <Col xs={24} lg={15}>
          ...
          >
            <Descriptions column={2} size="small">
              ...
              <Descriptions.Item label="最高学历">{detail.highest_education || '-'}</Descriptions.Item>
              ...
              <Descriptions.Item label="教育经历" span={2}>
                {detail.education.length
                  ? detail.education.map((item) => [item.school, item.degree, item.graduate_at]
                      ...
                      if (parsed.parse_status === 'system') {
                        ...
                        if (canManage) {
                          openResumeEditor({
                            ...
                            age: fields.age ?? detail.age,
                            highest_education: fields.highest_education || detail.highest_education,
                            major: fields.major || detail.major,
